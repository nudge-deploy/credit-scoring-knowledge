...
Instalasi:
  pip install flask flask-cors pandas numpy scikit-learn xgboost lightgbm joblib anthropic pillow
...
def run_predict(payload: dict) -> dict:
    if model is None:
        return {"error": "Model tidak tersedia"}, 503
...
def extract():
    """
    Upload satu dokumen dan ekstrak field-nya.
    Form fields:
      - file     : file gambar (PNG/JPG/WEBP)
...
def extract_and_predict():
    """
    Upload beberapa dokumen sekaligus + field manual tambahan,
    gabungkan semua hasil ekstraksi, lalu jalankan prediksi.

    Form fields (semua opsional, minimal ada satu):
      - ktp                : file gambar KTP
      - sku                : file gambar SKU/NIB
      - rekening_koran     : file gambar rekening koran
      - laporan_keuangan   : file gambar laporan keuangan
      - manual_fields      : JSON string field-field tambahan
    """
    ...
    # Proses setiap dokumen yang diupload
    for dk in doc_keys:
    ...
    # Isi default untuk field wajib yang belum ada
    defaults = {
        ...
        "nilai_agunan":300_000_000,"rasio_agunan_terhadap_plafon":1.5,"dti_ratio":0.3,
    }
...
if __name__ == "__main__":
    print("\n" + "═"*52)
    print("  API Kredit UMKM  →  http://localhost:5000")
    print("  Endpoints:")
    print("    GET  /               health check")
    print("    POST /predict        inferensi JSON")
    print("    POST /extract        ekstrak satu dokumen")
    print("    POST /extract-and-predict  upload + predict")
    print("═"*52 + "\n")
